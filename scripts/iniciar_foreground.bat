...
if /I "%SATELITE_RUNTIME_CLASSPATH_AVAILABLE%"=="true" (
    "%JAVA_EXECUTABLE%" -cp "%SATELITE_CLASS_PATH%" com.example.satelite.SateliteApplication "--debug=false" "--APP_SCHEDULER_ENABLED=false" "--APP_CICLO_UNICO=true" "--APP_ETL_REPESCAGEM_ENABLED=false" "--APP_PPG_ENABLED=%APP_PPG_ENABLED%" "--APP_VEDACIT_ENABLED=%APP_VEDACIT_ENABLED%" "--APP_SELIA_ENABLED=%APP_SELIA_ENABLED%" "--APP_SUPPORTE_ENABLED=%APP_SUPPORTE_ENABLED%" "--SELIA_NFE_WHITELIST_ENABLED=false" "--server.port=0" "--spring.main.web-application-type=none"
) else (
    "%JAVA_EXECUTABLE%" -jar "%JAR_PATH%" "--debug=false" "--APP_SCHEDULER_ENABLED=false" "--APP_CICLO_UNICO=true" "--APP_ETL_REPESCAGEM_ENABLED=false" "--APP_PPG_ENABLED=%APP_PPG_ENABLED%" "--APP_VEDACIT_ENABLED=%APP_VEDACIT_ENABLED%" "--APP_SELIA_ENABLED=%APP_SELIA_ENABLED%" "--APP_SUPPORTE_ENABLED=%APP_SUPPORTE_ENABLED%" "--SELIA_NFE_WHITELIST_ENABLED=false" "--ETL_INCREMENTAL_LOOKBACK_HOURS=0" "--server.port=0" "--spring.main.web-application-type=none"
)
